...
// function _getClassName( props, ...classNames ) {
...
// 	}
// 	if ( props.type ) {
// 		if ( !_.includes( [ "reset", "submit", "vertical", "horizontal" ], props.type ) ) {
...
// 		, render() {
// 			const props = { className: _getClassName( this.props, _CLASS_BUTTON, this.props.notext ? _CLASS_ICON_NOTEXT : _CLASS_SHADOW ), ..._.omit( this.props, [ "align", "corners", "icon", "iconAlign", "notext", "type" ].concat( omitKeys ) ) };
...
// 	const InputButton = _getButtonClass( "InputButton", [ "ref" ] );
...
// 	, mixins: [ ReactPureRenderMixin ]
// 	, render() {
// 		if ( _.includes( [ "button", "reset", "submit" ], this.props.type ) ) return (
...
// 		const refCallback = _getRefCallback( this.props.refCallback, "textinput", options );

// 		const divProps = { className: _getClassName( { type, ...this.props }, `${_CLASS_INPUT}-${type}`, _CLASS_BODY, `${_CLASS_SHADOW}-inset` ) };
...
// 	if ( __MOBILE__ ) {
// 		const NativeTransition = React.createClass( {
// 			componentWillAppear() {
// 				const options = {
